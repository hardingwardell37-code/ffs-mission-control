...
export function createAgentIntelligenceProvider(): AgentIntelligenceProvider | null {
  ...
  return { async propose(command, context) { try {
    const response = await fetch(`${config.baseUrl}/chat/completions`, { method: "POST", signal: AbortSignal.timeout(8_000), headers: { authorization: `Bearer ${config.apiKey}`, "content-type": "application/json" }, body: JSON.stringify({ model: config.model, temperature: 0, messages: [
      { role: "system", content: "Act only as the registered Mission Control agent described by context. Use its role, department, assigned skills, current task, tools, and governance. Propose; never execute. Challenge unsupported instructions, clarify missing facts, and delegate work outside the role. Return only schema JSON without chain-of-thought." }, { role: "user", content: JSON.stringify({ command, context }) },
    ], response_format: { type: "json_schema", json_schema: { name: "agent_proposal", strict: true, schema: proposalSchema } } }) });
    if (!response.ok) return null; const payload = await response.json() as { choices?: Array<{ message?: { content?: string } }> }; const content = payload.choices?.[0]?.message?.content;
    return content ? { proposal: JSON.parse(content) as AgentProposal, provider: config.identifier, model: config.model } : null;
  } catch { return null; } } };
}
