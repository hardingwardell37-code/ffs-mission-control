import "server-only";
import { AgentProviderError, createAgentIntelligenceProvider } from "@/lib/agent-intelligence-provider";
...
export async function invokeAgentIntelligence(command: string, context: AgentIntelligenceContext): Promise<AgentIntelligenceResult> {
  ...
  const governed = governAgentProposal(providerFailure ? unavailableProposal(providerFailure) : result?.proposal ?? null, context);
